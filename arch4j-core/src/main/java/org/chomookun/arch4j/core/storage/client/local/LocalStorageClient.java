...
public class LocalStorageClient extends StorageClient {
    ...
    public LocalStorageClient(Properties properties) {
        super(properties);
    ...
    public List<StorageResource> getChildResources(String resourceId) {
        ...
        if (childFiles != null) {
            for (File childFile : childFiles) {
                StorageResource resource = new LocalStorageResource(location, childFile);
    ...
    public StorageResource getParentResource(String resourceId) {
        ...
        if (parentFile != null) {
            StorageResource parentResource = new LocalStorageResource(location, parentFile);
    ...
    public StorageResource getResource(String resourceId) {
        File file = new File(location, resourceId);
        return new LocalStorageResource(location, file);
    }
    ...
    public StorageResource createFolderResource(String parentResourceId, String name) {
        ...
        return new LocalStorageResource(location, directory);
    }
    ...
    public StorageResource createFileResource(String parentResourceId, String name, InputStream inputStream) {
        ...
        if (!directory.exists()) {
            if (!directory.mkdirs()) {
                throw new RuntimeException(String.format("parentResourceId[%s] creation error", parentResourceId));
            }
        }
